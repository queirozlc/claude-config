---
name: orca-cli
description: "Operate Orca-managed worktrees, terminals, artifacts, automations, handoffs, and its embedded browser. Use Computer Use for desktop UI outside that browser."
---
...
Use the public `orca` CLI to operate Orca-managed worktrees, folder contexts, terminals, repos, automations, artifacts, skill sharing, worktree comments, and the browser embedded inside the Orca app. Use when the request names the orca CLI, an Orca worktree or terminal, an Orca artifact or shared skill, the browser inside Orca, or hands ownership of the work to another agent or worktree. Prefer this over raw `git worktree`, ad hoc PTYs, Playwright, or Computer Use when the task touches Orca-managed state. Use Computer Use for browser windows, webviews, or desktop UI outside Orca's embedded browser.
